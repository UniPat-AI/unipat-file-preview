...
组件默认内置 `ArchivePlugin`，基于浏览器原生 `DecompressionStream` 纯前端只读解析：

- `.tsv.gz`、`.csv.gz`、`.json.gz` 等单文件 GZIP：解压后自动根据去除 `.gz` 后的文件名复用内置的表格、JSON、文本插件。
- `.zip`：先解析目录结构，支持文件夹层级浏览与返回上一级，点击具体文件后才流式解压目标文件；支持 ZIP 内部包含 GZIP。
- `.tar`、`.tar.gz`、`.tgz`：完整支持 USTAR、PAX 路径与 GNU 长文件名格式，展示包内目录并可点击预览。
- **安全防线**：仅在浏览器内存中解压缩，不执行脚本；严格拒绝路径穿越、绝对路径、符号链接及损坏文件头。
